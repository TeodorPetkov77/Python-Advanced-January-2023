def students_credits(*args):
    ...
    for course in args:
        course = course.split("-")
        c_name = course[0]
        c_max_points = float(course[2])
        diyan_points = float(course[3])
        obtained_credits = float(course[1]) * (diyan_points / c_max_points)
        course_dict[c_name] = obtained_credits
    results = []
    for key, value in sorted(course_dict.items(), key=lambda x: (-x[1])):
        results.append(f"{key} - {value:.1f}")
    results = '\n'.join([x for x in results])
    credits_sum = sum(course_dict.values())
    if credits_sum >= 240:
        return f"Diyan gets a diploma with {credits_sum:.1f} " \
               f"credits.\n{results}"
    else:
        return f"Diyan needs {240 - credits_sum:.1f} " \
               f"credits more for a diploma.\n{results}"
...
# Basic Algebra - 7.5
# Comment
# First, we get the data for the Computer Science course. The total credits for this course are 12, and Diyan manages to reach 250 points out of 300 total points on the test. We calculate what percentage of the test Diyan took -> 250 / 300 = 83.3%. After that, we find the credits that he has for this course -> 12 * 83.3% = 10.
# Next, we get the data for the Basics Algebra course. Diyan manages to get 200/400 points on the test and receives 7.5 credits.
